...
Customer **getCustomers(FILE *database){
    ...
    if(strlen(string) == 0){
        ...
        while(token != NULL){ // tokenize the string into its individual customers
...
//function gets the index of the customer array from the customer's ID#
int getIndex(char *ID){

        return atoi(ID) % 200;
}

//find the number of categories from the categories argument string
...
//function that each thread calls, processing the orders for customers in each individual category
void *processCategory(void *arg){
    ...
    int match;
    while(queueSize != 0){ //loop through queue attaining each order matching the category 
                 
        match = strcmp(category, Order->category);
        if(match == 0){ //the order matches the thread category  
            tempCustomer = customerList[getIndex(Order->ID)]; //find customer from array
            if(tempCustomer->balance >= atof(Order->price)){ //customer has enough money to purchase order
               	...
               	tempCustomer->purchasedList = insertToQueue(tempCustomer, purchased, true);
            ...
            }else{ //customer does not have enough movey to purchase item
                ...
                rejected->title = Order->title;
             	...
             	tempCustomer->rejectedList = insertToQueue(tempCustomer, rejected, false);
...
//function prints out the final report of the transaction
...
//function frees all allocated memory for customers
...
char *getCategories(FILE *cat){//makes a string of the categories from the categories text file 
	
        char c;
        char *string = (char*)malloc(sizeof(char));
        string = "";
    	c = getc(cat);
    	while(c != EOF){//puts all file contents into one string   
	...
	char *token, *hold, *seps, *final;
        ...
        seps = " |\n";
        token = strtok_r(string, seps, &hold);//tokenize string 
	final = strdup(token);
	while(token != NULL){
		strcat(final, " ");
		strcat(final, token);
	...
	return strdup(final); //formatted string putting all categories in one line with a space separating them 
